...
def artist_fan_group_songs(request, artist_fan_group_name):
    songs = Song.objects.filter(artist_fan_group__artist_fan_group_name=artist_fan_group_name, artist_fan_group__user=request.user).distinct()

    if request.method == "POST":
        song_id = list(request.POST.keys())[1]
        song_group = Artist_Fan_Group.objects.filter(artist_fan_group_name=artist_fan_group_name, song__id=song_id, user=request.user)
        song_group.delete()
        messages.success(request, "Song removed from Artist Fan Group!")

    context = {'artist_fan_group_name': artist_fan_group_name,'songs': songs}

    return render(request, 'musicapp/artist_fan_group_songs.html', context=context)
...
def artist_fan_group(request):
    ...
    if len(request.GET) > 0:
        ...
        return render(request, 'musicapp/artist_fan_group.html', context)
